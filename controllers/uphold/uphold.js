...
const requiresLogin = async (req, res, next) => {
  if (!req.session.upholdAccessToken) {
    res
      ...
      .json(errorObj('Please login to ComputeEx using Uphold'));
  } else {
...
router.post('/record-wallet-address', async (req, res) => {
  if (!req.body.walletAddress) {
    return res
      ...
      .json(errorObj('Please send walletAddress in body'));
  }
  if (!isHexString(req.body.walletAddress)) {
    return res
      ...
      .json(
        errorObj(
          `Wallet address should be hex string: ${req.body.walletAddress}`
        )
      );
  }
...
router.get('/wallet-address', requiresLogin, async (req, res) => {
  if (!req.session.walletAddress) {
    ...
    if (walletAddress !== null) {
      ...
      return res.status(HTTP_STATUS.SUCCESS.OK).json(successObj(null));
    }
  }
  res
    .status(HTTP_STATUS.SUCCESS.OK)
    .json(successObj(req.session.walletAddress));
});
...
router.get('/generate-state', (req, res) => {
  randomBytes(16, (err, buffer) => {
    ...
    res.status(HTTP_STATUS.SUCCESS.OK).json(successObj(req.session.state));
  });
});
...
router.post('/login', async (req, res) => {
  ...
  res.status(HTTP_STATUS.SUCCESS.ACCEPTED).json(successObj(userObj));
});
...
router.get('/user', requiresLogin, async (req, res) => {
  ...
  const user = await sdk.getMe();
  await upholdModel.insertOrUpdateUser(user, req.session.walletAddress);
  res.status(HTTP_STATUS.SUCCESS.ACCEPTED).json(successObj(user));
});
...
router.get('/transaction', requiresLogin, async (req, res) => {
  ...
  if (transaction.userId !== req.session.upholdUserId) {
    return res
      .status(HTTP_STATUS.CLIENT.FORBIDDEN)
      .json(errorObj('Restricted'));
  }

  res.status(HTTP_STATUS.SUCCESS.OK).json(successObj(transaction));
});

router.get('/transactions', requiresLogin, async (req, res) => {
  const transactions = await upholdModel.getTransactions(
    req.session.upholdUserId
  );

  res.status(HTTP_STATUS.SUCCESS.OK).json(successObj(transactions));
});
...
router.post('/commit-transaction', requiresLogin, async (req, res) => {
  ...
  try {
    const transaction = await upholdModel.getTransaction(
    ...
    const output = await sdk.commitCardTransaction(...Object.values(args));
