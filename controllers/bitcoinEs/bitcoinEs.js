...
router.get('/btc-amount-available', async (req, res) => {
  if (!req.query.btcAmount) {
    return res
      ...
      .json(errorObj('Please send btcAmount as query parameter'));
  }
  ...
  if (isNaN(btcAmount) || (decimals && decimals.length > 8)) {
    return res
      .status(HTTP_STATUS.CLIENT.BAD_REQUEST)
      .json(errorObj("btcAmount that you've sent is invalid"));
  }
  ...
  res.status(HTTP_STATUS.SUCCESS.OK).json(successObj(boolean));
});
...
router.post('/register-deposit', async (req, res) => {
  if (!isWalletAddress(req.body.walletAddress)) {
    return res
      .status(HTTP_STATUS.CLIENT.BAD_REQUEST)
      .json(errorObj('invalid wallet address'));
  }

  if (!req.body.btcAmount || isNaN(Number(req.body.btcAmount))) {
    return res
      .status(HTTP_STATUS.CLIENT.BAD_REQUEST)
      .json(errorObj('btcAmount is not available'));
  }
  ...
  const btcNotAvailableError = () =>
    res
      .status(HTTP_STATUS.CLIENT.NOT_ACCEPTABLE)
      .json(errorObj('btcAmount is not available'));
  ...
  if (result.affectedRows === 1) {
    res.status(HTTP_STATUS.SUCCESS.OK).json(successObj(true));
  } else {
...
router.get('/btc-deposit-address', async (req, res) => {
  res
    ...
    .json(successObj(process.env.BITCOINES_DEPOSIT_WALLET));
});
...
router.get('/get-requests', async (req, res) => {
  ...
  const result = await bitcoinModel.getUserTransactions(
    req.query.walletAddress
  );
  res.status(HTTP_STATUS.SUCCESS.OK).json(successObj(result));
});
